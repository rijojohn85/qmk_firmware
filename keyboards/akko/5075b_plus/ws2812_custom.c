// Local copy of QMK's ChibiOS ws2812_spi driver for the Akko 5075B Plus.
//
// The only change: QMK's WS2812_SPI_DIVISOR macro chain accepts powers of two only,
// but on WB32 the value is written straight into SPI BAUDR (SCK = PCLK / value).
// PCLK2 here is 96MHz and the stock firmware uses 24 -> 4MHz, giving the textbook
// WS2812 timing (T0H 0.25us, T1H 0.75us). The nearest power of two, 32, yields 3MHz
// and a 1.0us T1H, which overruns the WS2812 '1' high time and desyncs the chain.
// So the power-of-two chain is dropped and the divisor is passed through verbatim.
